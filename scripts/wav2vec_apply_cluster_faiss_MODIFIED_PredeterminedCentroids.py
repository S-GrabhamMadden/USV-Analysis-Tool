...
from wav2vec_cluster_faiss_MODIFIED import parse_faiss_specs, Wav2VecFeatureReader
...
def get_iterator(args):
    ...
    with open(osp.join(args.data, f"{args.split}.tsv"), "r") as fp:
        ...
        def iterate():
            start = 0
            actualFeatures = np.load(args.data+f"/{args.split}.npy")
            with open(args.data+f"/{args.split}.lengths", "r") as lf:
                lengths = lf.readlines()

            for fname, lbl, len in zip(files, lbls, lengths):
                length = int(len)
                end = start+length
                feats = actualFeatures[start:end]
                start += length
                file = osp.join(root, fname.split("\t")[0])
                #feats = reader.get_feats(file).data
                #Modification Here:
                actualFeatures = np.load(args.data+f"/{args.split}.npy")
...
def main():
    ...
    centroids = np.load("largeWorkDir/outputData/CLUS16/centroids.npy")
