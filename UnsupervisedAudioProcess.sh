...
python scripts/wav2vec_cluster_faiss_MODIFIED.py $tgt_dir/${train_split}.tsv \
--checkpoint $model --save-dir $tgt_dir -f "CLUS16" --sample-pct 1.0

for split in $all_splits; do
  python scripts/wav2vec_apply_cluster_faiss_MODIFIED.py $tgt_dir \
  --checkpoint $model --path $tgt_dir/CLUS16 --split $split
done

python fairseq/examples/wav2vec/unsupervised/scripts/pca.py $tgt_dir/${train_split}.npy --output $tgt_dir/pca --dim $dim
